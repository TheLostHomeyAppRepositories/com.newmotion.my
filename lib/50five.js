...
class FiftyFiveClient {
        ...
        return await (async () => {
            ...
            try {
                ...
                if (sessionCookie) {
                    ...
                    //Warm up the session, we do not use the result but must not leave the rejection unhandled
                    this.getMyChargePoints()
                        .catch(err => console.log('⚠️ Warm-up chargepoint fetch failed: '+err.message));
                    return this.auth_token.api_cookie
                } else {
                    this.clearAuthCookie();
    ...
    {
        if (!this.auth_token.cred_url) {
            console.log('⚠️ No valid auth token available for card list');
            return [];
        }
